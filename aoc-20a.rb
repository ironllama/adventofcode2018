...
furthest = 0
# total_dists = []
# Too slow doing multiple times for each end-room. Just scan whole map once and
# record when one of the end rooms is encountered for the "furthest" amount.
# single_exits.each do |k,v|
# goal = v
...
while queue.length > 0
    curr = queue.shift
    # puts "Q: #{curr[0]} POS: #{curr[1]} GOAL: #{goal}"
    # if curr[1] == goal  # Found goal!
    #     total_dists.push(curr[0])
    #     break
    # end
    # unless single_exits.key(curr[1]).nil? && curr[0] > furthest
    if curr[0] > furthest
        # total_dists.push(curr[0])
        furthest = curr[0]
    end
